        ...
        <form class="navbar-search navbar-search-dark form-inline mr-3 d-none d-md-flex ml-lg-auto">
          <div class="form-group mb-0">
            <div class="input-group input-group-alternative">
              ...
              <input class="form-control" placeholder="Search Order" type="text" name="search">
            </div>
          </div>
        </form>
        <!-- User -->
        ...
        <div class="col">
          <div class="card shadow">
            <div class="card-header border-0" style="display: flex; flex-direction: row; justify-content: space-between;">
              <h3 class="mb-0">Order tables: ${message_search}</h3>
            </div>
            <div class="table">
              <table class="table align-items-center table-flush">
                ...
                <tbody>
                	<c:forEach var="o" items="${orders}">
                		<tr>
		                    <th scope="row">
		                    	<span class="mb-0 text-sm">#${o.id}</span>
		                    </th>
		                    <td>
		                      ${o.user_order.username}
		                    </td>
		                    <td>
		                      ${o.fullName}
		                    </td>
		                    ...
		                    <td>
		                      	<c:choose>
									<c:when test="${o.status==0}"><span class="badge badge-dot"><i class="bg-warning"></i>Chờ xác nhận</span></c:when>
									<c:when test="${o.status==10}"><span class="badge badge-dot"><i class="bg-danger"></i>Chờ xác nhận (Khách yêu cầu huỷ)</span></c:when>
									<c:when test="${o.status==13}"><span class="badge badge-dot"><i class="bg-danger"></i>Chờ lấy hàng (Khách yêu cầu huỷ)</span></c:when>
									<c:when test="${o.status==2}"><span class="badge badge-dot"><i class="bg-default"></i>Đã huỷ</span></c:when>
									<c:when test="${o.status==3}"><span class="badge badge-dot"><i class="bg-primary"></i>Chờ lấy hàng</span></c:when>
									<c:when test="${o.status==4}"><span class="badge badge-dot"><i class="bg-info"></i>Đang vận chuyển</span></c:when>
									<c:when test="${o.status==5}"><span class="badge badge-dot"><i class="bg-success"></i>Đã giao</span></c:when>
								</c:choose>
		                    </td>
		                    <td>
		                      <fmt:formatDate value="${o.createdDate}" pattern="dd/MM/yyyy"/>
		                    </td>
		                    ...
		                    <td class="text-right">
		                      <div class="dropdown">
		                        ...
		                        <div class="dropdown-menu dropdown-menu-right dropdown-menu-arrow">
		                          <a class="dropdown-item" href="orders/detail/${o.id}.htm">Xem chi tiết</a>
		                          <a class="dropdown-item" href="orders/xacnhan/${o.id}.htm">Xác nhận đơn hàng</a>
		                          <a class="dropdown-item" href="orders/vanchuyen/${o.id}.htm">Đặt là đang vận chuyển</a>
		                          <a class="dropdown-item" href="orders/dagiao/${o.id}.htm">Đặt là đã giao</a>
		                          <a class="dropdown-item" href="orders/huydon/${o.id}.htm">Huỷ đơn hàng</a>
		                          <a class="dropdown-item" onclick="theFunction(h = 'orders/xoadon/${o.id}.htm');">Xoá đơn hàng</a>
		                        </div>
		                      </div>
		                    </td>
	                  </tr>
	  ...
	  <div class="modal-dialog modal-dialog-centered" role="document">
	    <div class="modal-content">
	      ...
	      <div class="modal-body">
	        ...
	        Xoá đơn hàng sẽ không hoàn lại số lượng sản phẩm trong đơn hàng, hãy chắc chắn đơn hàng đã được huỷ trước khi xoá bỏ.
	      </div>
